 ...
 * Updated 01/30/2022 - auth. Scott Graham, dsu.edu, added gfx_xsize(), gfx_ysize() to retrive window size in case it changed
*/
...
void gfx_fillrect( int x1, int y1, int x2, int y2 )
{
    ...
    int width=x2-x1; width=(width>=0)?width:-width;
    int height=y2-y1; height=(height>=0)?height:-height;
...
char gfx_getKey() {
    if ( gfx_event_waiting() ) {
        char key = gfx_wait();
        while ( gfx_event_waiting() ) { gfx_wait(); } // flush out any repeat-keys
        return key;
    }
}
/*
