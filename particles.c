...
void add_particle(vec2 pos, vec2 vel, float mass){
	particle temp = {
		.pos = pos,
		.vel = vel,
		.mass = mass,
	};
	arr_push(&particle_list,&temp);
}
...
void gen_random_particles(size_t num){
	for(size_t i = 0; i < num; i++){
		vec2 randompos = {
			.x = (float)(randsign()*(rand() % 40)),
			.y = (float)(randsign()*(rand() % 40)),
		};
		vec2 randomvel = {
			.x = (float)(randsign()*(rand() % 10000000)),
			.y = (float)(randsign()*(rand() % 10000000)),
		};
		float randommass = (float)(rand() % 10000000);
		add_particle(randompos, randomvel, randommass);
	}	
}
...
void draw_particle(particle part){
	...
	int xscreen_part = xscreen_cam - (int)round(scale*(campos.x - part.pos.x));
	int yscreen_part = yscreen_cam + (int)round(scale*(campos.y - part.pos.y));
...
int main()
{
	...
	/*
	add_particle((vec2){15000,0}, (vec2){0,4000}, 50);
	add_particle((vec2){0,0}, (vec2){0,0}, 1000);
	add_particle((vec2){-12000,0}, (vec2){0,4000}, 2);
	*/

	gen_random_particles(100);
	...
	while(1) {
		...
		if(c != 0) {
			...
			if(c=='['){scale *= 2;}
			if(c==']'){scale *= 0.5;}
			if(c=='w'){campos.y += 400*invscale;}
			if(c=='a'){campos.x -= 400*invscale;}
			if(c=='s'){campos.y -= 400*invscale;}
			if(c=='d'){campos.x += 400*invscale;}
			if(c=='t'){
				campos.x = gfx_xClick();
				campos.y = gfx_yClick();
			}
